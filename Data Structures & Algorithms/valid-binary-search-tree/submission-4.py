...
class Solution:
    def isValidBST(self, root: Optional[TreeNode], leftMin = None, rightMax = None) -> bool:
        if not root:
            return True
        if root.left and (root.left.val >= root.val or (leftMin and root.left.val <= leftMin)):
            return False
        if root.right and (root.right.val <= root.val or (rightMax and root.right.val >= rightMax)):
            return False
